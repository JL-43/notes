 ...
 *  In T-SQL and many other SQL dialects that support pivot, the syntax requires an aggregation function--commonly 
 *  sum, count, avg, etc.--when using the pivot clause. 
 *  There isn't a "no-aggregation" variant in T-SQL's built-in pivot operator, 
 *  so if you don't need an aggregation, you usually have to simulate pivoting via other means 
 *  (e.g., conditional case expressions).
